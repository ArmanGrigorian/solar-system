...
export function PanelItem({
  ...
  return (
    <motion.div
      ...
      onDragEnd={onDragEnd}
      onClick={onClick}
      className={`nodrag absolute grid ${panel.width > panel.height ? "grid-cols-6 grid-rows-4" : "grid-cols-4 grid-rows-6"} gap-px p-px rounded-sm cursor-grab ${isActive ? "z-10 border-2 border-accent-primary" : "z-1 border border-white/20"} ${isError ? "bg-red-500/80" : "bg-[#1e3a8a]"}`}
      style={{
        x: panel.position.x,
        y: panel.position.y,
        width: panel.width,
      ...
      {isActive && (
        <div className="absolute -bottom-6.25 left-1/2 -translate-x-1/2 bg-bg-panel/90 backdrop-blur-sm text-text-primary px-1.5 py-0.5 rounded-sm text-[10px] whitespace-nowrap border border-border-color z-20">
          {panel.wattage}W
        </div>
      )}
    </motion.div>
